...
def create_facebook_account():
    ...
    if otp:
        # Verify OTP on Facebook page (update this part to complete registration)
        otp_url = "https://m.facebook.com/recover/code"
        otp_data = {
            "email": email,
            "otp": otp,
            "submit": "Confirm"
        }
        otp_response = session.post(otp_url, data=otp_data)
        if "confirmation" in otp_response.url:
